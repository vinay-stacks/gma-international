...
import { Menu, X, ChevronDown, ChevronUp, Mail, Phone, Facebook, Instagram, Youtube, MessageCircle, BookOpen, Award, Users } from "lucide-react"
...
const Header = () => {
  ...
  return (
    <header className="w-full fixed top-0 z-50">
      ...
      <div className="bg-gradient-to-r from-teal-600 via-teal-600 to-teal-600 text-white">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-center py-2">
            ...
            <div className="flex flex-wrap items-center justify-center lg:justify-start gap-4 lg:gap-6">
              <div className="flex items-center gap-2 text-blue-100 hover:text-white transition-colors duration-300">
                ...
                <span className="font-md">gmainternationaledu@gmail.com</span>
              </div>
              <div className="flex items-center gap-2 text-blue-100 hover:text-white transition-colors duration-300">
              ...
              <div className="marquee-container">
                <div className="marquee-content">
                  <span className="text-cyan-300 text-sm font-medium">
                    <marquee className="font-md text-white">🎓 Admissions Open 2025! Limited Seats Available | 📚 New AI & Robotics Programs | 🏆 Excellence in Education Since 2010</marquee>
                  </span>
                </div>
              </div>
            </div>
      ...
      {/* Main Header */}
      <div className={`bg-white/95 backdrop-blur-md transition-all duration-300 ${isScrolled ? 'shadow-lg border-b border-gray-200' : 'shadow-md'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-2">
            ...
            <div className="flex items-center space-x-4">
              <div className="relative group">
                <div className="">
                  <Image src="/images/gma-web-logo.webp" width={80} height={90} />
                </div>
              </div>
            ...
            <nav className="hidden custom-lg:flex items-center space-x-1">
              {navigationItems.map((item, index) => (
                <div key={index} className="relative group">
                  <a
                    href={item.href}
                    className="flex items-center space-x-2 text-gray-700 hover:text-blue-600 font-medium transition-all duration-200 py-3 px-4 rounded-xl hover:bg-blue-50 group"
                  >
                  ...
                  {/* Desktop Dropdown */}
                  {item.hasDropdown && (
                    <div className="absolute top-full left-0 w-64 bg-white shadow-2xl rounded-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50 border border-gray-100 mt-2 overflow-hidden">
                      <div className="py-3">
                        {item.dropdownItems.map((dropdownItem, dropdownIndex) => (
                          <div key={dropdownIndex} className="relative group/sub">
                            <a
                              href={dropdownItem.href}
                              className="flex items-center justify-between px-5 py-3 text-gray-700 hover:bg-blue-50 hover:text-blue-600 transition-all duration-200 border-b border-gray-50 last:border-b-0"
                            >
                            ...
                            {/* Sub Dropdown */}
                            {dropdownItem.hasSubDropdown && (
                              <div className="absolute top-0 left-full ml-2 w-56 bg-white shadow-2xl rounded-2xl opacity-0 invisible group-hover/sub:opacity-100 group-hover/sub:visible transition-all duration-300 z-[60] border border-gray-100 overflow-hidden">
                                <div className="py-3">
                                  {dropdownItem.subDropdownItems?.map((subItem, subIndex) => (
                                    <a
                                      key={subIndex}
                                      href={subItem.href}
                                      className="block px-5 py-3 text-gray-600 hover:bg-blue-50 hover:text-blue-600 transition-all duration-200 text-sm font-medium border-b border-gray-50 last:border-b-0"
                                    >
                                      {subItem.name}
                                    </a>
                                  ))}
            ...
            <div className="flex items-center space-x-4">
              ...
              <button
                ...
                className="custom-lg:hidden p-2 rounded-lg hover:bg-gray-100 transition-colors relative group"
                aria-label="Toggle mobile menu"
              >
                <div className="relative w-6 h-6">
                  <Menu
                    className={`w-6 h-6 text-gray-700 absolute transition-all duration-300 ${isMobileMenuOpen ? "opacity-0 rotate-180" : "opacity-100 rotate-0"}`}
                  />
                  <X
                    className={`w-6 h-6 text-gray-700 absolute transition-all duration-300 ${isMobileMenuOpen ? "opacity-100 rotate-0" : "opacity-0 -rotate-180"}`}
                  />
                </div>
              </button>
            </div>
              ...
              <div key={index} className="border-b border-gray-100 last:border-b-0">
                <div className="flex items-center justify-between">
                  ...
                  {item.hasDropdown && (
                    ...
                    >
                      {openDropdowns[index] ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                    </button>
                  )}
                </div>
                      ...
                      <div key={dropdownIndex}>
                        <div className="flex items-center justify-between">
                          <a
                            ...
                            onClick={!dropdownItem.hasSubDropdown ? toggleMobileMenu : undefined}
                          >
                            {dropdownItem.name}
                          </a>
                          {dropdownItem.hasSubDropdown && (
                            <button
                              onClick={() => toggleSubDropdown(index, dropdownIndex)}
                        ...
                        {/* Mobile Sub Dropdown */}
                        {dropdownItem.hasSubDropdown && openSubDropdowns[`${index}-${dropdownIndex}`] && (
          ...
          <div className="pt-6 border-t border-gray-200">
            <div className="space-y-4 mb-6">
              <div className="flex items-center space-x-3 p-4 bg-blue-50 rounded-xl">
                ...
                <span className="text-sm font-medium text-gray-700">gmainternationaledu@gmail.com</span>
              </div>
              <div className="flex items-center space-x-3 p-4 bg-blue-50 rounded-xl">
                <Phone className="w-5 h-5 text-teal-600 flex-shrink-0" />
                <span className="text-sm font-medium text-gray-700">+91-9319061527</span>
              </div>
            </div>

            {/* Mobile Social Icons */}
            <div>
              <p className="text-gray-700 text-sm mb-4 font-semibold">Follow Us:</p>
